...
- **Firmware `head` module** drives pan + tilt via `write_position`. Values come from `stackchan_core::Pose` after the `HeadFromEmotion` / `IdleHeadDrift` / `HeadFromAttention` modifier stack resolves
