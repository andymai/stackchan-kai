...
//!
//! A "tap" (in the hardware sense: a FT6336U rising-edge) is published
//! to `entity.input.tap_pending` by the firmware's touch task. The next
//! [`Modifier::update`] tick:
//!
//! 1. consumes the flag (sets `entity.input.tap_pending = false`);
//! 2. advances `entity.mind.affect.emotion` to the next variant in
//!    [`EMOTION_ORDER`];
//! 3. sets `entity.mind.autonomy.manual_until` to `now + MANUAL_HOLD_MS`
//!    (see [`MANUAL_HOLD_MS`]) so [`super::EmotionCycle`] stands down;
//! 4. clears an expired `manual_until` on any later tick, handing
//!    autonomy back to `EmotionCycle`.
//!
//! The tap event itself is *not* stored with a timestamp — the modifier
//! just observes "a tap is pending" via `entity.input`. The hardware
//! polling task can flag a tap at any time between render ticks; the
//! avatar state only changes when the render loop picks it up on the
//! next `update` call.
//!
//! ## Sim-testability
//!
//! Tests drive the modifier the same way firmware does: set
//! `entity.input.tap_pending = true`, then call `update(&mut entity)`
//! and assert on the resulting `entity.mind.affect.emotion` +
...
/// Modifier that advances `entity.mind.affect.emotion` on each pending
/// tap and gates `EmotionCycle` by writing
/// `entity.mind.autonomy.manual_until`.
///
...
impl Modifier for EmotionTouch {
    fn meta(&self) -> &'static ModifierMeta {
        static META: ModifierMeta = ModifierMeta {
            ...
            priority: -100,
            reads: &[Field::Emotion, Field::Autonomy, Field::TapPending],
            writes: &[Field::Emotion, Field::Autonomy, Field::TapPending],
        };
        &META
    }
...
mod tests {
    ...
    fn single_tap_advances_emotion_and_sets_hold() {
        ...
        assert!(
            ...
            "modifier must clear input.tap_pending after consuming it"
        );
    }
