...
pub const TOOLS_LIST_RESULT_JSON: &str = concat!(
    ...
    r#"{"name":"set_face_target","description":"Aim the avatar's head at a normalised 2D screen target (x and y in [-1.0, 1.0]; (0,0) centre, (-1,-1) bottom-left, (1,1) top-right). Use when an agent has already mapped a target into avatar-screen coordinates. For pan/tilt degrees use look_at instead.","inputSchema":{"type":"object","properties":{"x":{"type":"number","minimum":-1,"maximum":1},"y":{"type":"number","minimum":-1,"maximum":1},"hold_ms":{"type":"integer"}},"required":["x","y"]}},"#,
    r#"{"name":"set_camera_mode","description":"Toggle the LCD's display mode between avatar view (false) and live camera passthrough (true). Display-only — face tracking continues in either mode. Ephemeral: a reboot returns to avatar view regardless.","inputSchema":{"type":"object","properties":{"active":{"type":"boolean"}},"required":["active"]}},"#,
