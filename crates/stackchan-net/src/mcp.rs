...
pub const TOOLS_LIST_RESULT_JSON: &str = concat!(
    ...
    r#"{"name":"get_crash","description":"Return the most recent panic log captured by the boot path from the persistent RTC crash latch. Returns an empty result when no crash has been recorded since the last clear. Use to diagnose a recent reboot.","inputSchema":{"type":"object","properties":{}}},"#,
