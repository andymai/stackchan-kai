...
pub const TOOLS_LIST_RESULT_JSON: &str = concat!(
    ...
    r#"{"name":"schedule_motion","description":"Schedule a canonical one-shot motion (greet / nod / shake / laugh) to play in N seconds. Lifts the same scheduler create_reminder uses; returns the same id space. Use list_reminders to see both phrase-bound reminders and motion-bound ones — the motion entries carry a 'motion' field instead of 'phrase'. cancel_reminder cancels either kind by id. Runtime-only — does not survive reboot.","inputSchema":{"type":"object","properties":{"fire_in_secs":{"type":"integer","minimum":1,"maximum":432000},"motion":{"type":"string","enum":["greet","nod","shake","laugh"]}},"required":["fire_in_secs","motion"]}},"#,
    r#"{"name":"list_reminders","description":"Return the currently-scheduled entries. Each entry has an `id`, `fire_in_secs` (countdown), and either a `phrase` field (for create_reminder entries) or a `motion` field (for schedule_motion entries). Switch on whichever key is present.","inputSchema":{"type":"object","properties":{}}},"#,
    ...
    r#"{"name":"get_sensor_history","description":"Return the last 60 seconds of sensor snapshots, sampled at 1 Hz, oldest first. Each entry carries an 'age_secs' field (how long ago the sample was taken) plus the same field shape as get_sensors. Use to reason about recent physical context — 'was the avatar shaken in the last 10 seconds?', 'has the room gone dark since the last call?' — without needing a live subscription.","inputSchema":{"type":"object","properties":{}}},"#,
    ...
    r#"{"name":"set_behavior_flag","description":"Persist one boolean behaviour flag to /sd/STACKCHAN.RON: soliloquy_enabled (idle bubbles), hourly_chime_enabled (top-of-hour chirp), battery_icon_enabled (on-screen battery indicator), or toast_overlay_enabled (warn/error band at the bottom of the LCD). Each of these is captured at boot by its consuming task or modifier, so a reboot is required before the change takes effect; the success result echoes 'reboot_required: true' to make that explicit. Returns an InternalError with one of three details on failure: 'no SD card mounted' (no SD), 'config snapshot unavailable' (boot hasn't loaded config yet), or 'config write failed' (SD write error mid-transaction). On the last two the cache stays at the previous value rather than partially applying.","inputSchema":{"type":"object","properties":{"field":{"type":"string","enum":["soliloquy_enabled","hourly_chime_enabled","battery_icon_enabled","toast_overlay_enabled"]},"value":{"type":"boolean"}},"required":["field","value"]}},"#,
